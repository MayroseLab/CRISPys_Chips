...
def create_cas_offinder_input_file(list_of_candidates, cas_offinder_path, genome_path,
                                   max_number_of_mismatches=4, pam='NGG', equal2guide=True) -> str:
    """
    :param list_of_candidates: A list of CandidateWithOffTargets objects
    :param cas_offinder_path: A path where the input file for cas-offinder will be created
    :param genome_path: path to the genome file where off-targets are searched for.
    ...
    :param equal2guide: if True, in case where  max_number_of_mismatches is smaller the the number of mismach in a guide,
                        it will set the number of mismatches to the maximum the mismatches in the all guides
    :return: A path to the input for cas-offinder and will write the input file to cas_offinder_path
    """
    cas_offinder_infile = os.path.join(cas_offinder_path, 'cas_offinder_infile.txt')
    out = f"{genome_path}\n{'N' * 20}{pam}\n"
    for candidate in list_of_candidates:  # go over each candidate instead
        if equal2guide:
            # this will change the number of mismatches to be the highest of max_number_of_mismatches or
            # number of mismacthes between the guide and target.
            number_of_mismatches = max_number_of_mismatches
            for gene in candidate.targets_dict:
                candidate.targets_dict[gene].sort(key=lambda x: len(x[1]))

                if len(candidate.targets_dict[gene][0][1]) > number_of_mismatches:
                    number_of_mismatches = len(candidate.targets_dict[gene][0][1])
            out += f"{candidate.seq} {number_of_mismatches}\n"
        else:
            out += f"{candidate.seq} {max_number_of_mismatches}\n"
    with open(cas_offinder_infile, 'w') as f:
        f.write(out)
    return cas_offinder_infile


def run_cas_offinder(list_of_candidates: List, crispys_output_path: str, genome_path: str,
                     equal2guide: bool, max_number_of_mismatches: int = 4,
                     pam: str = 'NGG') -> List[List[str]]:
    """
    This function runs cas-offinder and returns its input
    :param equal2guide: if True, in case where  max_number_of_mismatches is smaller the the number of mismach in a guide,
                        it will set the number of mismatches to the maximum the mismatches in the all guides
    :param pam: A string containing the desired protospacer adjacent motif (PAM), using the IUPAC
    nucleotide codes (e.g. 'NRG' matches both NGG and NAG).
    ...
    :param max_number_of_mismatches: The maximum number of mismatches allowed between a sgRNA
     and a potential off-target.
    :return: The output of cas-offinder as a nested list, where each element is a potential target.
    """
    cas_offinder_path = f"{crispys_output_path}/cas_offinder"
    os.makedirs(cas_offinder_path, exist_ok=True)
    # create an casoffinder input file
    cas_offinder_infile = create_cas_offinder_input_file(list_of_candidates, cas_offinder_path,
                                                         genome_path, max_number_of_mismatches, pam, equal2guide)
    # run casoffinder without output file (read from stdout)
    off_targets = subprocess.run([globals.casoffinder_path, cas_offinder_infile, "C", "-"],
                                 capture_output=True, text=True)
    cas_offinder_output = [line.split("\t") for line in off_targets.stdout.split("\n")]
    cas_offinder_output.pop()
    print(f"Number of off-targets: {len(cas_offinder_output)}")
    return cas_offinder_output


def add_cas_off_targets(cas_offinder_results: List, sequence_to_candidate_dict: Dict):
    """
    This function adds all found off-targets to each CandidateWithOffTargets using the cas-offinder results
    :param cas_offinder_results: The output of cas-offinder as a nested list, where each element is a potential target.
    :param sequence_to_candidate_dict: sequence -> a CandidateWithOffTargets object with the proper sequence
    :return: None
    """
    for off_target_line in cas_offinder_results:
        off_target = OffTarget(off_target_line[3].upper(), off_target_line[1], int(off_target_line[2]),
                               off_target_line[4], int(off_target_line[5]))
        sgRNA_seq = off_target_line[0]

        candidate = sequence_to_candidate_dict[sgRNA_seq]
        if 'N' not in off_target.seq:
