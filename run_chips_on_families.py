...
def run(code_path: str, main_folder_path: str, crispys_name: str = "crispys_output", chips_name: str = "chips",
        ...
        restriction_site: str = "None", min_sg_per_gene: int = 4, chips_output_name: str = "chips"):
    """
    ...
    for family in families:
        ...
        if os.path.isdir(fam_path) and not family.startswith("."):
            ...
            command = f"cd {fam_path}\npython {code_path}/chips_main.py -crispys {fam_path} " \
                      ...
                      f"{n_singletons} -scoring {scoring} -restriction {restriction_site} -min_sg {min_sg_per_gene}"
...
if __name__ == '__main__':
    run(code_path="/groups/itay_mayrose/udiland/CRISPys_Chips",
         ...
         crispys_name="moff_0.15", chips_name="chips_moff0.15_4sg", n_mm=4, n_groups=20, n_best=5, n_grna=2, n_singletons=5,
         scoring="moff", restriction_site="GGTCTC", min_sg_per_gene=4)
